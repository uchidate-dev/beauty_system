    ...
    <div class="py-12 bg-[#fafafa] min-h-screen">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                ...
                <div class="text-left md:text-right">
                    ...
                    <div class="flex flex-wrap md:justify-end gap-2">
                        @foreach ($menus as $menu)
                            <span
                                class="text-[11px] text-gray-700 font-medium border border-gray-300 px-4 py-1.5 bg-white rounded-sm shadow-sm">
                                {{ $menu->name }}
                            </span>
                        @endforeach
                    </div>
            ...
            <form action="{{ route('reservations.store') }}" method="POST" id="reservation-form">
                ...
                {{-- カレンダー操作 --}}
                <div
                    class="bg-white border-t border-x border-gray-100 rounded-t-3xl p-6 flex justify-between items-center shadow-sm">
                    <button type="button" id="prev-week" onclick="changeWeek(-7)"
                        class="text-[10px] tracking-widest text-gray-400 hover:text-black transition disabled:invisible">
                        &lt; PREVIOUS WEEK
                    </button>
                    <span id="current-month" class="text-sm font-medium tracking-[0.4em] text-gray-800"></span>
                    <button type="button" onclick="changeWeek(7)"
                        class="text-[10px] tracking-widest text-gray-400 hover:text-black transition">
                        NEXT WEEK &gt;
                    </button>
                </div>
                        ...
                        <table class="w-full border-collapse text-center table-fixed min-w-[800px]">
                            <thead>
                                <tr id="calendar-header"
                                    class="bg-gray-50/50 border-b border-gray-100 text-[10px] text-gray-400">
                                    {{-- JS描画 --}}
                                </tr>
                            </thead>
                ...
                <div id="selection-summary"
                    class="fixed bottom-10 left-1/2 transform -translate-x-1/2 z-50 w-[95%] max-w-lg hidden opacity-0 translate-y-10 transition-all duration-500">
                    <div
                        class="bg-gray-900/95 backdrop-blur-sm text-white p-6 rounded-2xl shadow-[0_25px_50px_-12px_rgba(0,0,0,0.5)] flex flex-col gap-5">
                        ...
                        <button type="button" onclick="openModal()"
                            class="w-full bg-white text-gray-900 py-4 rounded-lg text-[10px] font-bold tracking-[0.5em] hover:bg-gray-100 transition active:scale-[0.98]">
                            CONFIRM RESERVATION
                        </button>
                    </div>
                </div>
                ...
                <div id="confirm-modal" class="fixed inset-0 z-[100] hidden flex items-center justify-center p-4">
                    ...
                    {{-- モーダル本体 --}}
                    <div class="relative bg-white w-full max-w-md p-8 rounded-3xl shadow-2xl transform scale-95 opacity-0 transition-all duration-300"
                        id="modal-content">
                        <h3
                            class="text-[10px] font-bold tracking-[0.5em] text-gray-400 uppercase mb-8 text-center border-b pb-4">
                        ...
                        <div class="space-y-6 mb-10">
                            ...
                            <div class="flex justify-between items-baseline">
                                ...
                                <span id="modal-display-dt"
                                    class="text-sm font-bold text-gray-900 tracking-tighter"></span>
                            </div>
                            <div class="pt-4 border-t border-dashed flex justify-between items-baseline">
                                <span class="text-[9px] text-gray-900 font-bold tracking-widest uppercase">Total
                                    Amount</span>
                                <span
                                    class="text-xl font-light text-gray-900">¥{{ number_format($menus->sum('price')) }}</span>
                            </div>
                        </div>

                        <div class="flex flex-col gap-3">
                            <button type="submit" form="reservation-form"
                                class="w-full bg-gray-900 text-white py-4 rounded-xl flex flex-col items-center justify-center hover:bg-black transition shadow-lg">
                            ...
                            </button> <button type="button" onclick="closeModal()"
                                class="w-full text-[9px] text-gray-400 tracking-widest py-2 hover:text-gray-900 transition uppercase">
                                Cancel
                            </button>
                        </div>
                    </div>
